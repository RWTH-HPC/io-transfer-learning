...
DATASET_DIR = r"/data/claix_dataset/data/claix"

CLAIX_MODEL_DIR = rf"/models/claix/cross_val/5-fold/from_scratch"
...
def run_kfold_with_seed(df_claix_posix, df_bandwidth, kfold_seed, config, device):
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(df_claix_posix, y=df_bandwidth)):
        ...
        run_fold(fold, config, df_claix_posix, df_bandwidth, train_ids, test_ids, device)



def run_fold(fold, config, df_claix_posix, df_bandwidth, train_ids, test_ids, device):
    config["fold"] = fold

    # Check if a saved model for this fold combo already exists -> don't rerun if yes
    claix_model_name = build_claix_model_name(config, config['fold'])
    ...
    if claix_model_path.exists():
        print(f"Found existing model for CLAIX fold {config['fold']}. Skipping...")
        return
...
def build_claix_model_name(config, fold):
    model_name = f"from_scratch_fold_{fold}_{config['loss']}"
