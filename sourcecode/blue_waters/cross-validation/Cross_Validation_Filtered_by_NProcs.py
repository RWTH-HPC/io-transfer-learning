...
KFOLD_SEED = 1474235857 # Changed manually

DATASET_DIR = r"/data/blue_waters_dataset"
...
def main():
    ...
    # ## Load the filtered nprocs data
    df_blue_waters_posix = pd.read_csv(DATASET_PATH)

    # ### Separate bandwidth from input features
    df_bandwidth = df_blue_waters_posix.pop('bandwidth')

    kfold = KFold(n_splits=config["num_folds"], shuffle=config["shuffle"], random_state=config["kfold_seed"])

    for fold, (train_ids, test_ids) in enumerate(kfold.split(df_blue_waters_posix, y=df_bandwidth)):
        print(f'FOLD {fold}')
        print('--------------------------------')

        run_fold(fold, config, df_blue_waters_posix, df_bandwidth, train_ids, test_ids, device)
...
def setup_dataflow(df_blue_waters_posix, df_bandwidth, train_ids, test_ids, batch_size, shuffle, device):
    # ### Setup training dataloader
    # Get the rows marked for training
    X_train = df_blue_waters_posix.iloc[train_ids]
    ...
    X_test = df_blue_waters_posix.iloc[test_ids]
...
def run_fold(fold, config, df_blue_waters_posix, df_bandwidth, train_ids, test_ids, device):
    config["fold"] = fold
    ...
    MODEL_DIR = Path(rf"/models/blue_waters/cross_val/5-fold-filtered-by-nprocs/{config.kfold_seed}")
    if not MODEL_DIR.exists():
        MODEL_DIR.mkdir()
    MODEL_PATH = Path(MODEL_DIR, MODEL_FILENAME)

    training_dataloader, test_dataloader = setup_dataflow(df_blue_waters_posix, df_bandwidth, train_ids, test_ids, config.batch_size, config.shuffle, device)
